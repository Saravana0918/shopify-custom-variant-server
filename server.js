...
async function uploadFileToShopify(base64, filename) {
  ...
  const data = await shopifyRequestGraphQL(query, variables);

  if (!data || !data.data || !data.data.fileCreate || !data.data.fileCreate.files[0]) {
    throw new Error("Shopify file upload failed: " + JSON.stringify(data));
  }

  return data.data.fileCreate.files[0].preview.image.url;
}
...
async function shopifyRequestGraphQL(query, variables = {}) {
  ...
  try { return JSON.parse(text); } catch (e) { throw new Error("Shopify GraphQL parse error: " + text); }
}
